...
#define if_likely(e)   if(!!(e))
    ...
    void Update(float deltaTime) {
        ...
        // Camera orbit
        if (Input::LeftMouseDown() && Input::KeyDown(ARK_KEY_LEFT_ALT)) {
        ...
        // Camera Pan
        if (Input::KeyDown(ARK_KEY_LEFT_CONTROL_GLFW) && Input::LeftMouseDown()) {
        ...
        if (!Input::KeyDown(ARK_KEY_LEFT_CONTROL_GLFW) && !Input::KeyDown(ARK_KEY_LEFT_ALT)) {
        ...
        if (Input::KeyPressed(ARK_KEY_BACKSPACE) && g_hoveredObjectIndex != -1) {
            // To do
        }
        ...
        // Open insert menu
        if (Input::KeyPressed(ARK_KEY_F2)) {
            Audio::PlayAudio(MENU_SELECT_AUDIO, MENU_SELECT_VOLUME);
    ...
    void EnterEditor() {
        ...
        g_yawAngle = std::atan2(delta.z, delta.x) + ARK_PI * 0.5f;
...
/* Rendering */
    ...
    void UpdateMenu() {
        ...
        // Create object menus
            ...
            if (g_selectedObjectType == PhysicsObjectType::DOOR && g_selectedObjectIndex != -1) {
                ...
                if (door) {
                    ...
                    g_menuItems.push_back({ "Rot Y", MenuItem::Type::VALUE_FLOAT, &door->m_rotation, ARK_PI * 0.5f, 5 });
                ...
                if (window) {
                    ...
                    g_menuItems.push_back({ "Rot Y", MenuItem::Type::VALUE_FLOAT, &window->m_rotationY, ARK_PI * 0.5f, 5 });
                }
            }
        ...
        if (Input::KeyPressed(ARK_KEY_W)) {
            Audio::PlayAudio(MENU_SELECT_AUDIO, MENU_SELECT_VOLUME);
            g_menuSelectionIndex--;
        }
        if (Input::KeyPressed(ARK_KEY_S)) {
            Audio::PlayAudio(MENU_SELECT_AUDIO, MENU_SELECT_VOLUME);
            g_menuSelectionIndex++;
        }
        ...
        // Interact with menu
        if (Input::KeyPressed(ARK_KEY_E) || Input::KeyPressed(ARK_KEY_ENTER)) {
            Audio::PlayAudio(MENU_SELECT_AUDIO, MENU_SELECT_VOLUME);
            MenuItem::Type& type = g_menuItems[g_menuSelectionIndex].type;
            Player* player = Game::GetPlayerByIndex(0);
            ...
            if (type == MenuItem::Type::INSERT_DOOR) {
                ...
                createInfo.rotation = ARK_PI * 0.5f;
