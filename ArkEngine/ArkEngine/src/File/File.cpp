...
namespace File {
    ...
    void MemCopyName(char* nameBuffer, const std::string& name) {
        std::memset(nameBuffer, 0, ARK_NAME_BUFFER_SIZE);
        std::memcpy(nameBuffer, name.data(), std::min((int)name.length(), ARK_NAME_BUFFER_SIZE - 1));
        nameBuffer[std::min((int)name.length(), ARK_NAME_BUFFER_SIZE - 1)] = '\0';
    }

    void MemCopyFileSignature(char* signatureBuffer, const std::string& signatureName) {
        std::memset(signatureBuffer, 0, ARK_SIGNATURE_BUFFER_SIZE);
    ...
    bool CompareFileSignature(char* signatureBuffer, const std::string& signatureName) {
        if (signatureName.length() >= ARK_SIGNATURE_BUFFER_SIZE) return false;

        char expectedSignature[ARK_SIGNATURE_BUFFER_SIZE];
        std::memset(expectedSignature, 0, ARK_SIGNATURE_BUFFER_SIZE);
        std::memcpy(expectedSignature, signatureName.data(), signatureName.length());
        return (std::memcmp(signatureBuffer, expectedSignature, ARK_SIGNATURE_BUFFER_SIZE) == 0);
    }
...
void File::ExportSkinnedModel(const SkinnedModelData& skinnedModelData) {
    std::string outputPath = "Resources/SkinnedModels" + skinnedModelData.name + ".skinnedmodel";
    ...
    std::memcpy(modelHeader.signature, "ARK_SKINNED_MODEL", 18);
...
SkinnedModelData File::ImportSkinnedModel(const std::string& filepath) {
    ...
    // Read 18 bytes for ARK_SKINNED_MODEL
...
SkinnedModelHeader File::ReadSkinnedModelHeader(const std::string& filepath) {
    ...
    char fileSignature[18];
    file.read(fileSignature, 18);
    if (std::strncmp(fileSignature, "ARK_SKINNED_MODEL", 18) != 0) {
        std::cout << "File::ReadSkinnedModelHeader() failed: 'ARK_SKINNED_MODEL' signature not found in header\n";
        return header;
    }
