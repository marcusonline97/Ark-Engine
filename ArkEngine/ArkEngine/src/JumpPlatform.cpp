...
namespace
{
    constexpr float BounceCooldownSeconds = 0.2f;
    const glm::vec3 BounceVelocity(0.0f, 7.5f, 0.0f);
}
...
{
    if (!obj ||
        m_bounceCooldownRemaining > 0.0f ||
    ...
    m_bounceCooldownRemaining = BounceCooldownSeconds;
}
