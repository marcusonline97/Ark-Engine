...
namespace MidiFileManager {
	...
	// Initialize the MIDI file manager and load the MIDI files 
    void Init() {
    ...
    void Update(float deltaTime) {
        // Play Goat
        if (Input::KeyPressed(KEY_NUM_LOCK)) {
            PlayTrack(0);
        }

        // Play Nocturne
        if (Input::KeyPressed(KEY_PAUSE)) {
            PlayTrack(1);
        }

        // Play Czardas
        if (Input::KeyPressed(KEY_PAGE_UP)) {
            PlayTrack(2);
        }

        // Stop
        if (Input::KeyPressed(KEY_END)) {
            Reset();
        }
