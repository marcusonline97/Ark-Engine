...
namespace Engine
{
    ...
    bool ArkEngine::Init(int width, int height)
    {
        ...
        Logging::Init() << "Scene and application types registered.";
        ...
        if (!m_editorUIActive)
        {
            Logging::Warning() << "Editor UI failed to initialize; continuing without editor UI.";
            if (glfwReady) ImGui_ImplGlfw_Shutdown();
            ImGui::DestroyContext();
        }
        else
        {
            Logging::Init() << "Editor UI initialized.";
        }

        const bool applicationInitialized = m_application->Init();
        if (applicationInitialized)
        {
            Logging::Init() << "ArkEngine initialized successfully.";
        }
        else
        {
            Logging::Warning() << "Application init returned false.";
        }
