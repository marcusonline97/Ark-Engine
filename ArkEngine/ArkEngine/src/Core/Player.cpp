...
void Player::Update(float deltaTime) {
	...
	float amt = 0.02f;
	if (Input::KeyDown(ARK_KEY_MINUS)) {
		_viewHeightStanding -= amt;
	}
	if (Input::KeyDown(ARK_KEY_EQUAL)) {
		_viewHeightStanding += amt;
	}

	// Crouching
	bool crouching = false;
	if (Input::KeyDown(ARK_KEY_LEFT_CONTROL_GLFW)) {
		crouching = true;
	}
	...
	_isMoving = false;
	if (Input::KeyDown(ARK_KEY_W)) {
		displacement -= _forward * speed;
		_isMoving = true;
	}
	if (Input::KeyDown(ARK_KEY_S)) {
		displacement += _forward * speed;
		_isMoving = true;
	}
	if (Input::KeyDown(ARK_KEY_A)) {
		displacement -= _right * speed;
		_isMoving = true;
	}
	if (Input::KeyDown(ARK_KEY_D)) {
		displacement += _right * speed;
		_isMoving = true;
	}
...
void Player::UpdateFirstPersonWeapon(float deltaTime) {
	...
	if (_currentWeaponIndex == Weapon::SHOTGUN) {
		...
		// Reload
		if (Input::KeyPressed(ARK_KEY_R) && false) {
			_weaponAction = RELOAD;
			_firstPersonWeapon.PlayAnimation("AKS74U_Reload", 1.1f);
			Audio::PlayAudio("AK47_Reload.wav", 1.0f);
		}
