...
void Scene::LoadMapData(const std::string& fileName) {
    ...
    if (!file.is_open() || !file.good()) {
        std::cout << "Map file '" << fullPath << "' not found. Creating empty scene with default objects.\n";
        return;
    }
    ...
    if (buffer.str().empty()) {
        std::cout << "Map file '" << fullPath << "' is empty.\n";
        return;
    }
    ...
    try {
        ...
        std::cout << "Failed to parse map file: " << e.what() << "\n";
        return;
    }

    // Load doors
    if (data.contains("doors") && data["doors"].is_array()) {
    ...
    if (data.contains("VolumesAdditive") && data["VolumesAdditive"].is_array()) {
    ...
    if (data.contains("VolumesSubtractive") && data["VolumesSubtractive"].is_array()) {
...
void Scene::LoadDefaultScene() {
    ...
    CleanUp();
    CreateDefaultSpawnPoints();
    CreateDefaultLight();  // Enable default lighting so objects are visible
    ...
    */
    ...
    //pictureFrame->SetRotationY(ARK_PI / 2);
    ...
    cushion0->SetRotationY(ARK_PI * -0.5f);
    ...
    cushion1->SetRotationY(ARK_PI * -0.5f);
    ...
    cushion2->SetRotationY(ARK_PI * -0.5f);
    ...
    cushion3->SetRotationY(ARK_PI * -0.5f);
    ...
    cushion4->SetRotationY(ARK_PI * -0.5f);
    ...
    for (int x = -3; x < 1; x++) {
        for (int y = -1; y < 5; y++) {
            for (int z = -1; z < 2; z++) {
                ...
                cube->SetRotationX(Util::RandomFloat(0, ARK_PI * 2));
                cube->SetRotationY(Util::RandomFloat(0, ARK_PI * 2));
                cube->SetRotationZ(Util::RandomFloat(0, ARK_PI * 2));
...
void Scene::CreateDefaultSpawnPoints() {
    ...
    }
    {
        ...
        spawnPoint.rotation = glm::vec3(-0.0, ARK_PI, 0);
    }
    {
        SpawnPoint& spawnPoint = g_spawnPoints.emplace_back();
...
void Scene::Update_OLD(float deltaTime) {
    ...
    if (Input::KeyPressed(ARK_KEY_SPACE) && false) {
            ...
            if (actor->userData) {
                ...
                if (physicsObjectData->type == PhysicsObjectType::RAGDOLL_RIGID) {
                    if (actor->userData) {
                        ...
                        transform.rotation.y = bullet.parentPlayersViewRotation.y + ARK_PI;
...
void Scene::LoadHardCodedObjects() {
    ...
    _toilets.push_back(Toilet(glm::vec3(11.2f, 0.1f, 3.65f), ARK_PI * 0.5f));
...
/*   CreateGameObject();
     ...
     glockAmmo->SetCollisionType(CollisionType::PICKUP);*/
        ...
        Game::SpawnPickup(PickUpType::GLOCK_AMMO, glm::vec3(0.40f, 0.78f, 4.45f), glm::vec3(0,ARK_PI * 0.4f, 0), true);

        for (int x = 0; x < 10; x++) {
            for (int z = 0; z < 10; z++) {

                glm::vec3 position = glm::vec3(1 + x * 0.4f, 0.5f, 1 + z * 0.4f);
                glm::vec3 rotation = glm::vec3(0, Util::RandomFloat(0, ARK_PI * 2), 0);
...

        {
            ...
            /*    GameObject& toilet = _gameObjects.emplace_back();
                 ...
                 toilet.SetRotationY(ARK_PI * 0.5f);
    ...
    glock.SetPosition(glm::vec3(2.5f, 1.5f, 3));
    glock.SetRotationY(ARK_PI * 0.5f);*/
     ...
     */
     ...
     dyingGuy.SetRotationX(ARK_PI * 0.5f);
     ...
     //unisexGuy.SetRotationX(ARK_PI * 0.5f);
 ...
 //   glock.SetMeshMaterial("SK_FPSArms_Female", "FemaleArms");
    ...
    shotgun.SetRotationY(ARK_PI * 0.5f);
   ...
   */
   ...
   //	aks74u.PlayAndLoopAnimation("AKS74U_ReloadEmpty", 0.1f);
       ...
       aks74u.SetRotationY(ARK_PI * 0.5f);   */
