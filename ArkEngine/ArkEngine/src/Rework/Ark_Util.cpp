...
char* ReadBinaryFile(const char* pFilename, int& size)
{
    ...
    if (error) {
        char buf[256] = { 0 };
        strerror_s(buf, sizeof(buf), err);
        ARK_ERROR("Error getting file stats: %s\n", buf);
        return NULL;
    }
    ...
    if (bytes_read != size) {
        char buf[256] = { 0 };
        strerror_s(buf, sizeof(buf), err);
        ARK_ERROR("Read file error file: %s\n", buf);
        exit(0);
    }
...
void ArkError(const char* pFileName, uint line, const char* format, ...)
{
    char msg[1000];
    va_list args;
