...
namespace Engine
{
    ...
    bool GameObject::RemoveComponent(Component* component)
    {
        ...
        auto it = std::find_if(m_components.begin(), m_components.end(), [component](const std::unique_ptr<Component>& element)
            {
    ...
    void ParseGLTFNode(cgltf_node* node, GameObject* parent, const std::filesystem::path& folder)
    {
        ...
        if (node->mesh)
        {
            for (cgltf_size pi = 0; pi < node->mesh->primitives_count; ++pi)
            {
                ...
                mat->SetShaderProgram(ArkEngine::GetInstance().GetGraphicsAPI().GetDefaultShaderProgram());
