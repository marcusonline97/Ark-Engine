...
public:
    ...
    Texture* GetPBRAlbedoTexture() const { return m_Materials.empty() ? nullptr : m_Materials[0].PBRmaterial.pAlbedo; };

    Texture* GetDiffuseTexture() const
    {
        if (m_Materials.empty())
            return nullptr;
        return m_Materials[0].pTextures[TEX_TYPE_BASE];
    }
